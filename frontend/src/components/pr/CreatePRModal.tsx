...
import { GitPullRequestCreateArrow, X, Search, ChevronDown } from "lucide-react";
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
        ...
        <div className="p-6 overflow-y-auto space-y-5 custom-scrollbar">
          ...
          <div className="relative">
            <label className="block text-xs font-semibold text-slate-700 mb-1">Assign Reviewers</label>
            <div
              onClick={() => setIsDropdownOpen(!isDropdownOpen)}
              className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm flex justify-between items-center cursor-pointer hover:bg-slate-100 transition-colors"
            >
              <span className={selectedReviewers.length > 0 ? "text-slate-800 font-medium" : "text-slate-400"}>
            ...
            {isDropdownOpen && (
              <div className="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl flex flex-col overflow-hidden">
